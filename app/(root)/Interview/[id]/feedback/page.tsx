...
const Feedback = async ({ params }: Props) => {
  const { id } = await params;
  const user = await getCurrentUser();
  ...
  // Debug log
  console.log("🔍 FEEDBACK DATA FROM FIRESTORE:", {
    id: feedback?.id,
    totalScore: feedback?.totalScore,
    ml_score: (feedback as any)?.ml_score,
    gemini_score: (feedback as any)?.gemini_score,
    agreement_level: (feedback as any)?.agreement_level,
  });

  // Extract ML data properly
  const mlScore = (feedback as any)?.ml_score || 0;
  const geminiScore =
    (feedback as any)?.gemini_score || feedback?.totalScore || 0;
  const agreementLevel = (feedback as any)?.agreement_level || "unavailable";
  const difference = (feedback as any)?.difference || 0;
  const confidence = (feedback as any)?.confidence || "";
